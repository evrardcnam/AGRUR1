$(function() {
    ...
    $("a#add").click(function() {
        if(!$("input#newName").val()) { showMessage("Erreur de saisie", "Veuillez indiquer un nom de commune.", "Retour"); return; }
        $.post('php/api.php?action=post_commune', {
            ...
            if(data.status == "200") {
                $(".rowtable").find(".row:last").before(
                    ...
                    '"><div class="col-xs-12 col-sm-9">' + $("input#newName").val() +
...
function updateEditLinks() {
    $("a.save").off().click(function() { // Enregistrement d'une modification en cours
        ...
        if(!$(row + ' .col-sm-9 input').val()) { showMessage("Erreur de saisie", "Veuillez indiquer un nom de commune.", "Retour"); return; }
        $.post('php/api.php?action=put_commune', {
            id: id,
            nom: $(row + ' .col-sm-9 input').val(),
    ...
    $("a.delete").off().click(function() { // Confirmer la suppression
        ...
        showConfirm({
            ...
            $.post('php/api.php?action=delete_commune', {
